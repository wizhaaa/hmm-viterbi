...
def evaluate_model(model, val_set, tags):
  ...
  val_text = val_set['text']  
  val_labels = val_set['NER']  
  ...
  for sentence in val_text:
      ...
      pred_tags = viterbi(model, sentence, tags)
      predicted_tags.append(pred_tags)

  
  flat_pred_tags = flatten_double_lst(predicted_tags)
  flat_true_labels = flatten_double_lst(val_labels)
  flat_indices = flatten_double_lst(val_indices)

  y_pred_dict = format_output_labels(flat_pred_tags, flat_indices)
  y_true_dict = format_output_labels(flat_true_labels, flat_indices)
