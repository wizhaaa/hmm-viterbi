...
def apply_smoothing(k, observation_counts, unique_obs):
    ...
    # 1. calc total count for each state
    state_totals = defaultdict(float)
    for (state, obs), count in observation_counts.items():
        state_totals[state] += count
    
    # 2. add-k smoothing + compute prob 
    log_smoothed_probs = {}
    for (state, obs), count in observation_counts.items():
        smoothed_count = count + k 
        # normalize: divide by total count for state + k * num of unique obs
        total = state_totals[states] + k * len(unique_obs)
